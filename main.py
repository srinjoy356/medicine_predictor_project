...
import os
from prediction_model import get_predicted_value, get_general_medications, helper, diabetes_model, heart_disease_model, parkinsons_model, sym_des
...
def home():
    try:
        ...
        if predicted_disease == "Unknown disease":
            return jsonify({'message': 'Unable to predict disease based on the given symptoms. Please provide more information.'}), 400
...
@app.route('/get_symptoms_1', methods=['GET'])
def get_symptoms_1():
    try:
        symptoms_array_1 = sym_des.Symptom_1.unique()
        symptoms_array_1 = symptoms_array_1.tolist()
        return jsonify({'symptoms': symptoms_array_1}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400

@app.route('/get_symptoms_2', methods=['GET'])
def get_symptoms_2():
    try:
        symptoms_array_2 = sym_des.Symptom_2.unique()
        symptoms_array_2 = symptoms_array_2.tolist()
        return jsonify({'symptoms': symptoms_array_2}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400

@app.route('/get_symptoms_3', methods=['GET'])
def get_symptoms_3():
    try:
        symptoms_array_3 = sym_des.Symptom_3.unique()
        symptoms_array_3 = symptoms_array_3.tolist()
        return jsonify({'symptoms': symptoms_array_3}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400

@app.route('/get_symptoms_4', methods=['GET'])
def get_symptoms_4():
    try:
        symptoms_array_4 = sym_des.Symptom_4.unique()
        symptoms_array_4 = symptoms_array_4.tolist()
        return jsonify({'symptoms': symptoms_array_4}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400
